...
public class MockDispatcher extends Dispatcher {
    ...
    public MockResponse dispatch(RecordedRequest request) throws InterruptedException {
        System.out.println(String.format(Locale.ENGLISH, "Request: %s", request));
            ...
            if (regexMatches(pathRegex, request.getPath())) {
                ...
                if (!responseQueue.isEmpty()) {
                    ...
                    if (smartMockResponse.delayMs() > 0) {
                        System.out.println(String.format(Locale.ENGLISH, "Delaying response %s for %d ms (path: %s)", smartMockResponse, smartMockResponse.delayMs(), pathRegex));
                        Thread.sleep(smartMockResponse.delayMs());
                        System.out.println(String.format(Locale.ENGLISH, "Finished delaying response %s for %d (path: %s)", smartMockResponse, smartMockResponse.delayMs(), pathRegex));
                    }
