...
./gradlew publish -PbuildNumber="${BUILD_NUMBER}" -PmavenUrl="${MAVEN_URL}" || exit 1

echo "======== Generate pom.xml for custom publish"
./gradlew generatePomFileForMavenJavaPublication  -PbuildNumber="${BUILD_NUMBER}" || exit 1
